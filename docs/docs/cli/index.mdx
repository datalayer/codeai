...
Use `--agent-id` to pick a preconfigured agent spec from the [agentspecs catalogue](https://github.com/datalayer/agentspecs):
...
# Specific agent
codeai --agent-id crawler

# With codemode disabled
codeai --agent-id data-acquisition --no-codemode
```
...
|--------|-------|---------|-------------|
| `--agent-id` | `-a` | `data-acquisition` | Agent spec ID to start |
